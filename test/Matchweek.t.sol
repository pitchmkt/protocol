...
contract MatchweekTest is Test {
    ...
    function test_submitPrediction() public {
        ...
        assertEq(matchweek.entryStake(0), stake);
    ...
    function test_submitPrediction_variableStake() public {
        ...
        assertEq(matchweek.entryStake(entryId), stake);
        assertEq(matchweek.totalStaked(), stake);
        assertEq(stablecoin.balanceOf(address(matchweek)), stake);
        assertEq(stablecoin.balanceOf(ALICE), 1_000_000_000 - stake);
    }
    ...
    function test_claimPrize_multipleWinners_evenSplit() public {
        ...
        _publishResults();
        _commitTwoEntryDistribution(0, 1, 8, stake, stake);
    ...
    function test_claimPrize_proportionalSplit() public {
        ...
        _publishResults();
        _commitTwoEntryDistribution(0, 1, 8, aliceStake, bobStake);
    ...
    function test_commitDistribution_perfectTenReleasesCarryPool() public {
        ...
        vm.prank(BOB);
        uint256 entryId = matchweek2.submitPrediction(_buildValidPredictions(), stake);
    ...
    ///      using that entry's actual stake as the tier total. For a single-leaf tree, root = leaf
    ///      and proof = [].
    function _publishAndCommitSingleEntry(uint256 entryId, uint8 tier) internal {
        _publishResults();
        bytes32 root = _merkleLeaf(entryId, tier);
        uint256[5] memory totalStakePerTier_;
        totalStakePerTier_[tier - PrizeConfig.MIN_WINNING_TIER] = matchweek.entryStake(entryId);
        vm.prank(ADMIN);
        matchweek.commitDistribution(root, totalStakePerTier_);
    }
    ...
    ///      with the tier total set to the sum of their individual stakes.
    function _commitTwoEntryDistribution(uint256 entryA, uint256 entryB, uint8 tier, uint256 stakeA, uint256 stakeB)
        internal
    {
