...
///      there's no real stablecoin to point to, so a fresh ERC20Mock is deployed instead.
contract MatchweekFactoryScript is Script {
    ...
    ///      ERC20Mock since there's no real stablecoin locally; every other known chain ID maps
    ///      to a hardcoded address.
    function _resolveStablecoin() private returns (IERC20) {
        if (block.chainid == ANVIL_CHAIN_ID) {
            ERC20Mock mock = new ERC20Mock();
            console.log("Anvil detected - deployed ERC20Mock stablecoin at:", address(mock));
